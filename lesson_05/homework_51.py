...
target_cars = []
for car_name, car in car_data.items():
  year = car[1]
  engine = car[2]
  price = car[4]

  if year > search_criteria[0] and engine > search_criteria[1] and price > search_criteria[2]:
    target_cars.append((car_name, year, engine, price))

def get_price(car):
  return car[3]

sorted_by_price = sorted(target_cars, key=get_price)

print(target_cars)
print(sorted_by_price)
i = 0
for car in sorted_by_price[:5]:
  i += 1
  print(i, car)